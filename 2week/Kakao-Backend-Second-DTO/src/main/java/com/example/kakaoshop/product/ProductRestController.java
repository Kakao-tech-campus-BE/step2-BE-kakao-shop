...
import com.example.kakaoshop.product.response.ProductResponseFindAllDTO;
import com.example.kakaoshop.product.response.ProductResponseFindByIdDTO;
...
public class ProductRestController {
    ...
    public ResponseEntity<?> findAllProducts() {
        ...
        List<ProductResponseFindAllDTO> responseDTO = new ArrayList<>();

        // 상품 하나씩 집어넣기
        responseDTO.add(new ProductResponseFindAllDTO(
...
public ResponseEntity<?> findProductById(@PathVariable int id) {
    // 상품을 담을 DTO 생성
    ProductResponseFindByIdDTO responseDTO = null;
    ...
    if(id == 1){
        ...
        responseDTO = new ProductResponseFindByIdDTO(1, "기본에 슬라이딩 지퍼백 크리스마스/플라워에디션 에디션 외 주방용품 특가전", "", "/images/1.jpg", 1000, 5, optionDTOList);
        ...
        responseDTO = new ProductResponseFindByIdDTO(2, "[황금약단밤 골드]2022년산 햇밤 칼집밤700g외/군밤용/생율", "", "/images/2.jpg", 2000, 5, optionDTOList);
    }else {
