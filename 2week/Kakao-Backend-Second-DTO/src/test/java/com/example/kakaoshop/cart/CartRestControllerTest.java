...
public class CartRestControllerTest {
  ...
  public void findAll_test() throws Exception {
    ...
    // when
    ResultActions resultActions = mvc.perform(get("/carts"));
    ...
    resultActions.andExpect(
        jsonPath("$.response.products[0].cartItems[0].option.optionName")
            .value("01. 슬라이딩 지퍼백 크리스마스에디션 4종"));
    resultActions.andExpect(
        jsonPath("$.response.products[0].cartItems[0].option.price").value(10000));
