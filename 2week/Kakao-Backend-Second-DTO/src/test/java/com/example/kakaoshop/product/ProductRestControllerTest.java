...
public class ProductRestControllerTest {
  ...
  public void findAll_test() throws Exception {
    ...
    resultActions.andExpect(
        jsonPath("$.response[1].productName").value("[황금약단밤 골드]2022년산 햇밤 칼집밤700g외/군밤용/생율"));
  ...
  public void findById_test() throws Exception {
    ...
    // when
    ResultActions resultActions = mvc.perform(get("/products/" + id));
    ...
    resultActions.andExpect(
        jsonPath("$.response.options[0].optionName").value("01. 슬라이딩 지퍼백 크리스마스에디션 4종"));
    ...
    resultActions.andExpect(
        jsonPath("$.response.options[1].optionName").value("02. 슬라이딩 지퍼백 플라워에디션 5종"));
