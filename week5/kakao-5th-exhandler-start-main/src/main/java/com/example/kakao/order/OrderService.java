...
public class OrderService {
    ...
    public OrderResponse.FindByIdDTO mSave(User user) {
        ...
        cartJPARepository.mDeleteByUserId(user.getId());
    ...
    // 기본 delete
    @Transactional
    public OrderResponse.FindByIdDTO saveV2(User user) {
        List<Cart> cartList = cartJPARepository.mFindByUserId(user.getId());
    ...
    // item join fetch order join fetch option join fetch product
    ...
    // item join fetch order
    public OrderResponse.FindByIdDTO findByIdV2(int id, User user) {
        Order order = orderJPARepository.findById(id).orElseThrow(
                () -> new Exception400("해당 주문 내역을 찾을 수 없습니다.")
        );

        List<Item> itemList = itemJPARepository.mFindByOrderIdV2(id);
