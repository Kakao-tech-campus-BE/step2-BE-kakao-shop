...
@Transactional(readOnly = true) // 변경감지를 하지 않음
...
public class ProductService {
    ...
    public ProductResponse.FindByIdDTO findById(int id) {
        ...
        List<Option> optionListPS =
                optionJPARepository.findByProductId(productPS.getId());
        return new ProductResponse.FindByIdDTO(productPS, optionListPS);
    }

    public ProductResponse.FindByIdDTOv2 findByIdv2(int id) {
        List<Option> optionListPS =
                optionJPARepository.findByProductIdJoinProduct(id);
