...
public class UserService {
    ...
    public void join(UserRequest.JoinDTO requestDTO) {
        sameCheckEmail(requestDTO.getEmail()); //email 중복 확인

        requestDTO.setPassword(passwordEncoder.encode(requestDTO.getPassword())); //비밀번호 인코딩
        try {
            userJPARepository.save(requestDTO.toEntity()); //저장
        } catch (Exception e) {
            throw new Exception500("unknown server error");
        }
    }

    public String login(UserRequest.LoginDTO requestDTO) { //로그인
