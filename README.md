...
### AOP 적용해 유효성 검사 ###  
GlobalValidationHandler에서 요청된 데이터 유효성 검사를 위해 PostMapping 어노테이션에 PointCut을 등록하고, Advice 작성, 메서드 실행 전 실행될 로직이므로 @Before로 JoinPoint 적용한다. 이는 모든 PostMapping 메서드 실행 전에 요청된 데이터의 유효성 검사를 진행한다. 

### GlobalExceptionHandler 적용 ###   
@ControllerAdvice 를 사용해 GlobalExceptionHandler 작성하였다. Exception400, Exception401, Exception403, Exception404, Exception500와 이외의 Exception에 대 전역적으로 처리한다. 

### 장바구니 담기 기능 : Cart의 `addCartList()` ###  
1. 요청 데이터에 동일한 옵션이 있는 경우, 400 에러 발생
2. 요청 데이터에 존재하지 않는 옵션이 있는 경우, 404 에러 발생 
3. 정상 작동
   1. 장바구니에 이미 존재하는 옵션이 요청된 경우, 요청된 수량만큼 수량 추가(장바구니 업데이트)
   2. 새로운 옵션이 요청된 경우, 장바구니에 저장

### 장바구니 수정(주문) 기능 : Cart의 `update()` ###  
1. 장바구니가 비어있는 경우, 404 에러 발생
2. 요청 데이터가 비어있는 경우, 400 에러 발생
3. 요청 데이터에 동일한 장바구니 번호가 있는 경우, 400 에러 발생 
4. 요청 데이터에서 수량이 0 또는 음수일 경우, 400 에러 발생
5. 요청 데이터에 존재하지 않는 장바구니 번호가 있는 경우, 404 에러 발생
6. 정상 작동 : 장바구니 수량 수정
...
2. 정상 작동
   1. 주문 생성 후 저장
   2. 장바구니로 item 생성해 item 리스트 생성 후 저장
   3. 장바구니 삭제
 
### 주문결과 확인 기능 : Order의 `findById()` ###  
1. 존재하지 않는 주문을 조회한 경우, 404 에러 발생
2. 정상 작동 : 해당하는 주문 조회해 respondDTO 생성해 전달
