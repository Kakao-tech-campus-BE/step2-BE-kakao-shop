...
기존 API 문서에 안내된 내용에 약간의 수정을 하였습니다.
...
```
이메일 중복을 체크하는데, check만 있어 용도의 구분이 명확하게 드러나지 않습니다. 또한 체크는 현재 구현되지는 않았지만 구매자/판매자/관리자 체크를 위해서 사용할 수 있기 때문에, 명확한 목적 구분을 위해서 ```/check/email``` 로 수정하였습니다.
...
POST 메서드를 이용하므로 굳이 add를 다시 붙일 필요가 없기 때문에 끝에 있던 ```/add``` 를 제거하였습니다.
...
기존에 있는 장바구니의 내용을 업데이트 합니다. 서버의 기존 리소스를 업데이트 하는 과정이므로, POST 보다는 PUT 메서드를 사용하는 것이 적절해 보입니다.
...
기존 API에는, 장바구니를 삭제하는 기능이 없어서 추가했습니다. 장바구니 삭제는 기존에 있는 데이터를 삭제하는 과정이므로 DELETE 메서드를 사용했습니다.
...
주문을 저장하는 API 입니다. POST 메서드를 사용하고 있기 때문에 기존에 존재했던 ```/save``` 는 필요하지 않을 것 같아 제거하였습니다.
...
주문한 상품의 옵션을 수정하는 기능이 없어서 추가해본 API 입니다. body 부분에는 ```/carts/update``` 와 동일한 형태의 자료가 오면 될 것 같습니다. 주문 내역은 삭제되어서는 안되는 자료라고 생각됩니다. 따라서 주문이 최소된 경우라 하더라도 그 기록은 남아야 하기 때문에 ```/orders/``` 에서는 업데이트만 지원하고 DELETE는 지원하지 않는 방향이 좋을 것 같습니다.
