...
public class CartRestControllerTest {
    ...
    public void cartUpdate_test() throws Exception {
        ...
        String responseBody = resultActions.andReturn().getResponse().getContentAsString();
        System.out.println("테스트 : " + responseBody);
        System.out.print(resultActions);

        // verify
        resultActions.andExpect(jsonPath("$.success").value("true"));
        resultActions.andExpect(jsonPath("$.response.totalPrice").value(209000));
        resultActions.andExpect(jsonPath("$.response.cartItems[0].cartId").value(4));
        resultActions.andExpect(jsonPath("$.response.cartItems[0].optionId").value(1));
        resultActions.andExpect(jsonPath("$.response.cartItems[0].optionName").value("01. 슬라이딩 지퍼백 크리스마스에디션 4종"));
        resultActions.andExpect(jsonPath("$.response.cartItems[0].quantity").value(10));
        resultActions.andExpect(jsonPath("$.response.cartItems[0].price").value(100000));
    }
}
