...
public class CartService {
    ...
    @Transactional
    public void addCart(List<CartRequest.SaveDTO> requestDTOs, User sessionUser) {
        // 1. 동일한 옵션이 들어오면 예외처리
        ...
        if (count != actualCount) throw new Exception400("동일한 옵션을 담을 수 없습니다.");
        ...
        catch (Exception e) { throw new Exception500("상품을 담는 과정에서 오류가 발생했습니다."); }
    }
...
//    }
    ...
    public CartResponse.UpdateDTO updateCart(List<CartRequest.UpdateDTO> requestDTOs, User user) {
        ...
        int count = requestDTOs.size();
        int actualCount = (int) requestDTOs.stream()
                .mapToInt(requestDTO -> requestDTO.getCartId()).distinct().count();
        if (count != actualCount) throw new Exception400("동일한 장바구니 상품을 수정할 수 없습니다.");


        // 3. 유저 장바구니에 없는 cartId가 들어오면 예외처리
        Set<Integer> cartSet = cartList.stream().mapToInt(cart -> cart.getId()).boxed().collect(Collectors.toSet());
