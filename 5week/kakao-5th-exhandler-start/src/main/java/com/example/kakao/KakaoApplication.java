...
public class KakaoApplication {
	...
	CommandLineRunner localServerStart(UserJPARepository userJPARepository, ProductJPARepository productJPARepository, OptionJPARepository optionJPARepository, PasswordEncoder passwordEncoder){
		return args -> { // 여기 매개변수에 이렇게만 넣어도 dependency injection이 됨
			// 그리고 밑에 리턴되는 값들이 실행이 된다 commandlinerunner이면 실행시킨다

			// carts/add 담기하면 잘 담김
			// 그대로 save하면 uk 제약조건 오류가 걸린다
			// Cart에 있음 userid랑 optionid랑 동일한 것이 들어갈 수 없다

			// 숙제 2번 cartservice에서 잡아야 됨
			// 같은 optionid가 들어갔을 때 터지면 안됨
			// 안 들어가는 이유가 트랜잭션에서 터짐
	...
	private User newUser(String username, PasswordEncoder passwordEncoder){
		return User.builder()
				...
				.username(username)
				.roles(username.equals("admin") ? "R OLE_ADMIN" : "ROLE_USER")
				.build();
	}
