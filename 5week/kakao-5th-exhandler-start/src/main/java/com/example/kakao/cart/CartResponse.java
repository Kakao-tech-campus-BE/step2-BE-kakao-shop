    ...
    public static class FindAllDTOv2 {
        ...
        public class ProductDTO {
            ...
            public ProductDTO(Cart cart) {
                this.productId = cart.getOption().getProduct().getId(); // 1
                this.productName = cart.getOption().getProduct().getProductName(); // 2
                this.cartId = cart.getId();
                this.optionName = cart.getOption().getOptionName(); // 3
    ...
    public static class FindAllDTO {
        ...
        public FindAllDTO(List<Cart> cartList) {
            this.products = cartList.stream()
                    // 중복되는 상품 걸러내기
                    .map(cart -> cart.getOption().getProduct()).distinct()// 1
                    .map(product -> new ProductDTO(product, cartList)).collect(Collectors.toList()); // 2~3
        ...
        public class ProductDTO {
            ...
            public ProductDTO(Product product, List<Cart> cartList) {
                ...
                this.productName = product.getProductName(); // 2
                // 현재 상품과 동일한 장바구니 내역만 담기
                this.carts = cartList.stream()
                        .filter(cart -> cart.getOption().getProduct().getId() == product.getId()) // 3
                        .map(CartDTO::new)
