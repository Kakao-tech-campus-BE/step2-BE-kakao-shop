	...
	CommandLineRunner localServerStart(UserJPARepository userJPARepository, ProductJPARepository productJPARepository, OptionJPARepository optionJPARepository, PasswordEncoder passwordEncoder) {
		return args -> {
			...
			optionJPARepository.saveAll(Arrays.asList(
					...
					newOption(Product.builder().id(4).build(), "선택01_바른곡물효소 누룽지맛 2박스", 17900),
					...
					newOption(Product.builder().id(7).build(), "이너딜리티 1박스", 26800),
					...
					newOption(Product.builder().id(10).build(), "홍가리비2kg(50미이내)", 8900),
					...
					newOption(Product.builder().id(14).build(), "[특품] 샤인머스켓 1kg 1-2수", 12900),
