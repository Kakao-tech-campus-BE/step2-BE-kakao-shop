...
public class DummyEntity {
    ...
    //생성되는 카트 더미 자료: [{id:<int>,user:{User},option:{Option},quantity:<int>,price:<int>},{....},{....}]
    protected List<Cart> cartDummyList(User user, List<Option> optionListPS) {
        return Arrays.asList(
            newCart(user, optionListPS.get(3), 10),
            newCart(user, optionListPS.get(4), 5)
        );
    }

    //주문 목록의 더미리스트로, 하나의 주문에는 다양한 옵션이 들어있을 수 있기 때문에 Order테이블에는 option항목이 필요없다.
    //생성되는 더미자료: [{id:<int>,user:{User}},{....}]
    protected List<Order> orderDummyList(User user) {
        return Arrays.asList(
                newOrder(user),
                newOrder(user)
        );
    ...
    //카트에 있던 항목들을 유저의 0번째 주문목록에 넣는다.
    //생성되는 더미자료: [{id:<int>,user:{User},option:{Option},quantity:<int>,price:<int>},{....}]
    protected List<Item> itemDummyList(List<Cart> cartListPS, List<Order> orderListPS){
        return Arrays.asList(
