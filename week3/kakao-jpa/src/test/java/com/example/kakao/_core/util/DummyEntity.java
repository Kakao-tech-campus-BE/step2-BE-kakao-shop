...
public class DummyEntity {
    ...
    protected List<Cart> cartDummyList(User userPS, List<Option> optionListPS){
        return Arrays.asList(
                newCart(userPS, optionListPS.get(0), 5),
                newCart(userPS, optionListPS.get(1), 4)
        );
    }

    protected List<Item> orderItemDummyList(List<Cart> cartListPS, Order order){
        List<Item> dummyList = new ArrayList<>();
        for(Cart c : cartListPS){
            dummyList.add(newItem(c, order));
        }
        return dummyList;
    }
}
