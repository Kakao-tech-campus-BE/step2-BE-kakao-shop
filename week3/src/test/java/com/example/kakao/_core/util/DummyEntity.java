...
public class DummyEntity {
    ...
    protected Cart newCart(User user, Option option, Integer quantity){ //DB 반영
        return Cart.builder()
                .user(user)
                .option(option)
                .quantity(quantity)
    ...
    protected User userDummy = newUser("user1");


    protected List<Cart> cartDummyList(List<Option> optionList){
        return Arrays.asList(
                newCart(userDummy, optionList.get(0), 5), //id:1
                newCart(userDummy, optionList.get(1), 5) //id:2
        );
    }

    protected List<Order> orderDummyList(){
        return Arrays.asList(newOrder(userDummy)
        );
    }

    protected List<Item> itemDummyList(List<Cart> cartList){
        return cartList.stream().map(
                cart -> newItem(cart, newOrder(userDummy))
