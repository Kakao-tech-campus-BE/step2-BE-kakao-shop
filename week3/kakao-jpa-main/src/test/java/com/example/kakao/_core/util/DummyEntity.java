...
public class DummyEntity {
    ...
    protected User userDummy(){
        return newUser("ssar");
    }

    protected List<Order> orderDummyList(){
        return Arrays.asList(newOrder(newUser("ssar")));
    }

    protected List<Item> itemDummyList(List<Cart> cartList){
        return cartList.stream().map(
                cart -> newItem(cart, newOrder(cart.getUser()))
        ).collect(Collectors.toList());
    }

    protected List<Cart> cartDummyList(List<Option> optionList){
        return Arrays.asList(
                newCart(userDummy(), optionList.get(0), 5),
                newCart(userDummy(), optionList.get(1), 3)
        );
    }

    protected List<Cart> cartDummyList2(){
        return Arrays.asList(
                newCart(userDummy(), newOption(newProduct("기본에 슬라이딩 지퍼백 크리스마스/플라워에디션 에디션 외 주방용품 특가전", 1, 1000), "01. 슬라이딩 지퍼백 크리스마스에디션 4종", 10000), 5),
                newCart(userDummy(), newOption(newProduct("기 특가전", 2, 3000), "01. 슬라이딩 지퍼백 크리스마스에디션 4종", 10000), 5)
        );
    }
