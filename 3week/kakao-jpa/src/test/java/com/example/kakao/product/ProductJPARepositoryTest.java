...
public class ProductJPARepositoryTest extends DummyEntity {
    ...
    @BeforeEach
    public void setUp(){
        List<Product> productListPS = productJPARepository.saveAll(productDummyList()); //ProductListPS에 상품 더미데이터 저장
        optionJPARepository.saveAll(optionDummyList(productListPS));
        em.clear(); //영속화되어있으면 영속성 컨텍스트에 있는 것을 가져오기 때문에 clear를 해줘야함
    }
