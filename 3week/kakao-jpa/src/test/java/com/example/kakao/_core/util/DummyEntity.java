...
public class DummyEntity {
  ...
  protected Option newOption(Product product, String optionName, int price) {
    return Option.builder().product(product).optionName(optionName).price(price).build();
  }
  ...
  protected Item newItem(Order order, Option option, int quantity) {
    return Item.builder()
        .order(order)
        .option(option)
        .quantity(quantity)
  ...
  protected Order newOrder(User user) {
    return Order.builder().user(user).id(1).build();
  }
