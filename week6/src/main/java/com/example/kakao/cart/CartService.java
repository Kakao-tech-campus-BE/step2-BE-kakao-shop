...
public class CartService {
    ...
    @Transactional
    public void addCartList(List<CartRequest.SaveDTO> requests, User user) {
        List<Integer> sets = requests.stream().map(CartRequest.SaveDTO::getOptionId).distinct().collect(Collectors.toList());
        if (sets.size() != requests.size()) {
            throw new Exception400("잘못된 요청입니다. 요청에서 중복이 발생하였습니다.");
        }

        List<Integer> optionIds = requests
                .stream()
                .map(CartRequest.SaveDTO::getOptionId)
                .collect(Collectors.toList());

        List<Option> options = optionRepository.findAllById(optionIds);
        List<Cart> savedCarts = cartRepository.findByUserIdOrderByOptionIdAsc(user.getId()).orElse(null);
    ...
    @Transactional
    public CartResponse.UpdateDTO update(List<CartRequest.UpdateDTO> requests, User user) {
        if (requests.isEmpty()) {
            throw new Exception404("장바구니 업데이트 요청이 비어있습니다.");
        }

        List<Integer> sets = requests.stream().map(CartRequest.UpdateDTO::getCartId).distinct().collect(Collectors.toList());
        if (sets.size() != requests.size()) {
            throw new Exception400("잘못된 요청입니다. 요청에서 중복이 발생하였습니다.");
        }
