...
public interface CartJPARepository extends JpaRepository<Cart, Integer> {
  ...
  void mSave(@Param("userId") int userId, @Param("optionId") int optionId,
  ...
  Optional<Cart> findByOptionIdAndUserId(@Param("optionId") int optionId,
      @Param("userId") int userId);

  @Modifying
  @Query("update Cart c set c.quantity = :quantity, c.price = :price where c.id = :cartId")
  void updateQuantityAndPrice(@Param("cartId") int cartId, @Param("quantity") int quantity,
