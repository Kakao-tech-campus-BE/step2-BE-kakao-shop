...
public class SaveOrderUseCase {
    ...
    public OrderResponse execute(User user) {
        ...
        OrderEntity orderEntity = saveOrderEntity(user);
        List<OrderItemEntity> orderItemEntities = saveOrderItemEntity(orderItems, orderEntity);

        deleteCarts(cartEntities);

        return OrderResponseConverter.from(orderEntity, orderItemEntities, getTotalPrice(orderItems));
    }

    private OrderEntity saveOrderEntity(User user) {
        return orderRepository.save(OrderConverter.to(user));
    }
    ...
    private List<OrderItem> toOrderItem(List<CartEntity> cartEntities) {
        return cartEntities.stream()
                .map(x -> OrderItemConverter.to(x.getOption(), x.getQuantity()))
