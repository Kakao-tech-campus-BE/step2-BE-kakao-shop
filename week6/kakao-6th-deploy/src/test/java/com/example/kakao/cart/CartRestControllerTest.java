...
class CartRestControllerTest extends MyRestDoc {
    ...
    @DisplayName("장바구니 담기 성공: 이미 장바구니에 존재하는 옵션을 담았을 경우 수량 추가")
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    void addCartList_update_test() throws Exception {
        // given
        List<CartRequest.SaveDTO> requestDTOs = new ArrayList<>();
        CartRequest.SaveDTO item = new CartRequest.SaveDTO();
        item.setOptionId(1);
        ...
        resultActions.andExpect(jsonPath("$.success").value("true"));
        resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
        assertThat(cartJPARepository.findByOptionIdAndUserId(1, 1).get().getQuantity()).isEqualTo(10);
    }

    @DisplayName("장바구니 담기 실패: 중복된 옵션 요청")
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    void addCartList_duplicate_option_test() throws Exception {
        // given
        ...
        List<CartRequest.SaveDTO> requestDTOs = new ArrayList<>(List.of(item1, item2));
        String requestBody = om.writeValueAsString(requestDTOs);
    ...
    @DisplayName("장바구니 담기 실패: 존재하지 않는 옵션")
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    void addCartList_invalid_option_test() throws Exception {
        // given
        List<CartRequest.SaveDTO> requestDTOs = new ArrayList<>();
        CartRequest.SaveDTO item = new CartRequest.SaveDTO();
        item.setOptionId(1000);
        item.setQuantity(5);
        requestDTOs.add(item);
        String requestBody = om.writeValueAsString(requestDTOs);
    ...
    @WithUserDetails(value = "ssarmango@nate.com")
    @Transactional // deleteByUserId(1)을 수행하기 위해 Transactional을 붙여줌
    @Test
    void update_empty_cart_test() throws Exception {
        // given
    ...
    @DisplayName("장바구니 수정 실패: 중복된 옵션 요청")
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    void update_duplicate_option_test() throws Exception {
        // given
        ...
        item2.setCartId(1);
        item2.setQuantity(10);
        List<CartRequest.UpdateDTO> requestDTOs = new ArrayList<>(List.of(item1, item2));
        String requestBody = om.writeValueAsString(requestDTOs);
    ...
    @DisplayName("장바구니 수정 실패: 장바구니에 없는 아이템")
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    void update_not_existing_item_test() throws Exception {
        // given
        List<CartRequest.UpdateDTO> requestDTOs = new ArrayList<>();
        CartRequest.UpdateDTO item = new CartRequest.UpdateDTO();
        item.setCartId(100);
