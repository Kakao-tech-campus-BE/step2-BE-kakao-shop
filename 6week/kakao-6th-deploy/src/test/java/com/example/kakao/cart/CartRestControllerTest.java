...
public class CartRestControllerTest extends MyRestDoc {
    ...
    @DisplayName("장바구니 담기 실패 테스트 1 - 인증 필요")
    @Test
    public void addCartList_fail_test_unauthorized() throws Exception {
        // given -> optionId [1,2,16]이 teardown.sql을 통해 들어가 있음
        int optionId = 80;
        List<CartRequest.SaveDTO> requestDTOs = new ArrayList<>();
        CartRequest.SaveDTO item = new CartRequest.SaveDTO();
    ...
    @DisplayName("장바구니 담기 실패 테스트 2 - 동일한 옵션")
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    public void addCartList_fail_test_same_options() throws Exception {
        // given -> optionId [1,2,16]이 teardown.sql을 통해 들어가 있음
        List<CartRequest.SaveDTO> requestDTOs = new ArrayList<>();
        CartRequest.SaveDTO item = new CartRequest.SaveDTO();
        item.setOptionId(3);
        ...
        // verify
        resultActions.andExpect(jsonPath("$.success").value("false"));
        resultActions.andExpect(jsonPath("$.error.message").value("동일한 옵션을 담을 수 없습니다."));
        resultActions.andExpect(status().is(400));
        resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
    }

    @DisplayName("장바구니 담기 실패 테스트 3 - 존재하지 않는 옵션")
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    public void addCartList_fail_test_option_not_exist() throws Exception {
        // given -> optionId [1,2,16]이 teardown.sql을 통해 들어가 있음
        int optionId = 80;
        List<CartRequest.SaveDTO> requestDTOs = new ArrayList<>();
        CartRequest.SaveDTO item = new CartRequest.SaveDTO();
    ...
    @DisplayName("장바구니 조회 실패 테스트 - 인증 필요")
    @Test
    public void findAll_fail_test_unauthorized() throws Exception {
        // given teardown
    ...
    @DisplayName("장바구니 수정 실패 테스트 1 - 인증 필요")
    @Test
    public void update_fail_test_unauthorized() throws Exception {
        // given -> cartId [1번 5개,2번 1개,3번 5개]가 teardown.sql을 통해 들어가 있음
        List<CartRequest.UpdateDTO> requestDTOs = new ArrayList<>();
        CartRequest.UpdateDTO item = new CartRequest.UpdateDTO();
        item.setCartId(1);
    ...
    @DisplayName("장바구니 수정 실패 테스트 2 - 장바구니 비어있음")
    @WithUserDetails(value = "ssarorange@nate.com")
    @Test           // user2 : ssarorange
    public void update_fail_test_cart_empty() throws Exception {
        // given
        ...
        // verify
        resultActions.andExpect(jsonPath("$.success").value("false"));
        resultActions.andExpect(jsonPath("$.error.message").value("장바구니가 비어 있습니다."));
        resultActions.andExpect(status().is(400));
        resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
    }

    @DisplayName("장바구니 수정 실패 테스트 3 - 동일한 옵션 상품")
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    public void update_fail_test_options_same() throws Exception {
        // given -> cartId [1번 5개,2번 1개,3번 5개]가 teardown.sql을 통해 들어가 있음
        List<CartRequest.UpdateDTO> requestDTOs = new ArrayList<>();
        CartRequest.UpdateDTO item = new CartRequest.UpdateDTO();
        item.setCartId(1);
        ...
        // verify
        resultActions.andExpect(jsonPath("$.success").value("false"));
        resultActions.andExpect(jsonPath("$.error.message").value("동일한 장바구니 상품을 수정할 수 없습니다."));
        resultActions.andExpect(status().is(400));
        resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
    }

    @DisplayName("장바구니 수정 실패 테스트 4 - 존재하지 않는 장바구니 상품")
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    public void update_fail_test_cartId_not_exist() throws Exception {
        // given -> cartId [1번 5개,2번 1개,3번 5개]가 teardown.sql을 통해 들어가 있음
        int cartId = 80;
        List<CartRequest.UpdateDTO> requestDTOs = new ArrayList<>();
        CartRequest.UpdateDTO item = new CartRequest.UpdateDTO();
        ...
        // verify
        resultActions.andExpect(jsonPath("$.success").value("false"));
        resultActions.andExpect(jsonPath("$.error.message").value("존재하지 않는 장바구니 상품입니다 : " + cartId));
        resultActions.andExpect(status().is(404));
        resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
    }
