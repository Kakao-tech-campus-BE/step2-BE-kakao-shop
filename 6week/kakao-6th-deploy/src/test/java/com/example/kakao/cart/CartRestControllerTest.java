...
public class CartRestControllerTest extends MyRestDoc {
    ...
    //같은 option번호 넣을 경우에 수량 추가됨
    @DisplayName("장바구니 추가 같은 옵션 테스트")
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    public void addCartList_same_test() throws Exception {
        // given -> optionId [1,2,16]이 teardown.sql을 통해 들어가 있음
        List<CartRequest.SaveDTO> requestDTOs = new ArrayList<>();
        CartRequest.SaveDTO item = new CartRequest.SaveDTO();
        item.setOptionId(1);
    ...
    @DisplayName("장바구니 추가 에러 테스트")
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    public void addCartList_error_test() throws Exception {
        // given -> optionId [1,2,16]이 teardown.sql을 통해 들어가 있음
        Integer optionId = 50;
        List<CartRequest.SaveDTO> requestDTOs = new ArrayList<>();
        CartRequest.SaveDTO item = new CartRequest.SaveDTO();
    ...
    @DisplayName("장바구니 전체 조회 에러 테스트")
    @WithUserDetails(value = "nomango@nate.com")
    @Test
    public void findAll_error_test() throws Exception {
        // given teardown
    ...
    public void update_error_test() throws Exception {
        ...
        // verify
        resultActions.andExpect(jsonPath("$.success").value("false"));
        resultActions.andExpect(jsonPath("$.error.message").value("유저의 장바구니에 없는 cartId가 있습니다: "+cartId));
        resultActions.andExpect(jsonPath("$.error.status").value(404));
        resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
    }
