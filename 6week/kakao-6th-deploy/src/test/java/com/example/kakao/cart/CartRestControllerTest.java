...
public class CartRestControllerTest extends MyRestDoc {
    ...
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    public void addCartList_withWrongOption_test() throws Exception {
        // given -> optionId [1,2,16]이 teardown.sql을 통해 들어가 있음
        List<CartRequest.SaveDTO> requestDTOs = new ArrayList<>();
        CartRequest.SaveDTO item = new CartRequest.SaveDTO();
        ...
        resultActions.andExpect(MockMvcResultMatchers.jsonPath("$.error.message").value("존재하지 않는 옵션에 대한 요청이 들어왔습니다. : [" + optionId + "]"));
    ...
    @WithUserDetails(value = "ssarmango@nate.com")
    @Test
    public void update_withWrongCartId_test() throws Exception {
        // given
        List<CartRequest.UpdateDTO> requestDTOs = new ArrayList<>();
        CartRequest.UpdateDTO item = new CartRequest.UpdateDTO();
        Integer cartId = 123456; // 존재하지 않는 cartId
        ...
        // verify
        resultActions.andExpect(MockMvcResultMatchers.jsonPath("$.success").value("false"));
        resultActions.andExpect(MockMvcResultMatchers.jsonPath("$.error.message").value("존재하지 않는 장바구니에 대한 요청이 들어왔습니다. : " +cartId));
        resultActions.andExpect(MockMvcResultMatchers.jsonPath("$.error.status").value(404));
        resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
    }
