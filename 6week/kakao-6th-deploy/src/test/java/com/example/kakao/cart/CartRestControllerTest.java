...
public class CartRestControllerTest extends MyRestDoc {
  ...
  public void addCartList_sameOption_fail_test() throws Exception {
    ...
    resultActions.andExpect(
        jsonPath("$.error.message").value("동일한 옵션이 이미 담겨있습니다. : " + item.getOptionId()));
  ...
  @WithUserDetails(value = "ssarmango@nate.com")
  @Test
  public void addCartList_not_exist_option_fail_test() throws Exception {
    // given -> optionId [1,2,16]이 teardown.sql을 통해 들어가 있음
    List<CartRequest.SaveDTO> requestDTOs = new ArrayList<>();
    CartRequest.SaveDTO item = new CartRequest.SaveDTO();
    ...
    resultActions.andExpect(jsonPath("$.success").value("false"));
    resultActions.andExpect(
        jsonPath("$.error.message").value("해당 옵션을 찾을 수 없습니다 : " + item.getOptionId()));
  ...
  public void findAll_test() throws Exception {
    ...
    resultActions.andExpect(jsonPath("$.response.products[0].carts[0].option.optionName").value(
  ...
  @WithUserDetails(value = "ssarmango@nate.com")
  @Test
  public void update_same_fail_test() throws Exception {
    // given -> cartId [1번 5개,2번 1개,3번 5개]가 teardown.sql을 통해 들어가 있음
    List<CartRequest.UpdateDTO> requestDTOs = new ArrayList<>();
    CartRequest.UpdateDTO item = new CartRequest.UpdateDTO();
    item.setCartId(1);
    ...
    resultActions.andExpect(jsonPath("$.success").value("false"));
    resultActions.andExpect(
        jsonPath("$.error.message").value("동일한 상품이 이미 담겨있습니다. : " + item.getCartId()));
    resultActions.andExpect(jsonPath("$.error.status").value(400));
    resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
  }
  ...
  @WithUserDetails(value = "ssarmango@nate.com")
  @Test
  public void update_not_exist_fail_test() throws Exception {
    // given -> cartId [1번 5개,2번 1개,3번 5개]가 teardown.sql을 통해 들어가 있음
    List<CartRequest.UpdateDTO> requestDTOs = new ArrayList<>();
    CartRequest.UpdateDTO item = new CartRequest.UpdateDTO();
    item.setCartId(100);
    ...
    // verify
    resultActions.andExpect(jsonPath("$.success").value("false"));
    resultActions.andExpect(jsonPath("$.error.message").value("유저의 장바구니에 없는 상품 입니다."));
    resultActions.andExpect(jsonPath("$.error.status").value(400));
    resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
  }
