...
public class CartRestControllerTest extends MyRestDoc {
    ...
    @WithUserDetails(value = "ssarmango@nate.com")
    @DisplayName("장바구니 상품 추가시 동일 옵션 예외 발생 통합 테스트")
    @Test
    public void addCartList_sameOption_fail_test() throws Exception {
        ...
        resultActions.andExpect(jsonPath("$.error.message").value("동일한 옵션을 처리할 수 없습니다"));
        resultActions.andExpect(jsonPath("$.error.status").value("400"));
        resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
    }

    @WithUserDetails(value = "ssarmango@nate.com")
    @DisplayName("장바구니 조회하기 통합 테스트")
    @Test
    ...
    @Test
    public void update_sameCartIdInputs_fail_test() throws Exception {
        // given -> cartId [1번 5개,2번 1개,3번 5개]가 teardown.sql을 통해 들어가 있음
        List<CartRequest.UpdateDTO> requestDTOs = new ArrayList<>();
        CartRequest.UpdateDTO item = new CartRequest.UpdateDTO();
        item.setCartId(1);
        ...
        resultActions.andExpect(jsonPath("$.success").value("false"));
        resultActions.andExpect(jsonPath("$.response").isEmpty());
        resultActions.andExpect(jsonPath("$.error.message").value("잘못된 접근입니다. 동일한 장바구니 아이디는 중복해서 들어올 수 없습니다."));
        resultActions.andExpect(jsonPath("$.error.status").value("400"));
        resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
    }

    @WithUserDetails(value = "ssarmango@nate.com")
    @DisplayName("장바구니 업데이트시 유저 장바구니에 없는 cartId가 들어오면 예외 발생 통합 테스트")
    @Test
    public void update_notValidCartIdInputs_fail_test() throws Exception {
        // given -> cartId [1번 5개,2번 1개,3번 5개]가 teardown.sql을 통해 들어가 있음
        List<CartRequest.UpdateDTO> requestDTOs = new ArrayList<>();
        CartRequest.UpdateDTO item = new CartRequest.UpdateDTO();
        item.setCartId(5);
        ...
        resultActions.andExpect(jsonPath("$.success").value("false"));
        resultActions.andExpect(jsonPath("$.response").isEmpty());
        resultActions.andExpect(jsonPath("$.error.message").value("잘못된 접근입니다. 해당 유저가 가지고 있지 않는 장바구니에 접근할 수 없습니다."));
        resultActions.andExpect(jsonPath("$.error.status").value("400"));
        resultActions.andDo(MockMvcResultHandlers.print()).andDo(document);
    }
