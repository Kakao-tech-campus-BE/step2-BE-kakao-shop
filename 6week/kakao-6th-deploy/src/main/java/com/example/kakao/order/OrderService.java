...
public class OrderService {
    ...
    public OrderResponse.SaveDTO save(User user) {
        ...
        if (carts.isEmpty())
            throw new Exception404("장바구니에서 주문할 상품이 없습니다.");

        Order order = Order.builder()
                .user(user).build();

        try { orderJPARepository.saveAndFlush(order);
        } catch (Exception e) {
        ...
        try { itemJPARepository.saveAllAndFlush(items);
        } catch (Exception e) {
            throw new Exception500("주문 상품을 저장하는 과정에서 오류가 발생했습니다.");
        }

        try { cartJPARepository.deleteByUserId(user.getId());
        } catch (Exception e) {
    ...
    public OrderResponse.FindByIdDTO findById(User user, int id) {
        ...
        if (order.getUser().getId() != user.getId())
            throw new Exception403("주문에 접근 권한이 없습니다.");

        List<Item> items = itemJPARepository.findByOrderId(id);
        if (items.isEmpty())
            throw new Exception404("해당 주문의 상품을 찾을 수 없습니다.");
