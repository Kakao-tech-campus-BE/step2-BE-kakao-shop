...
public class UserRestControllerTest {
    ...
    public void updatePassword_test() throws Exception {
        ...
        updatePasswordDTO.setPassword("sonny1234!");
        String requestBody = om.writeValueAsString(updatePasswordDTO);
        User user = User.builder().id(1).roles("ROLE_USER").build();
        String jwt = JWTProvider.create(user);

        // stub
        Mockito.doNothing().when(userService).updatePassword(any(UserRequest.UpdatePasswordDTO.class),any());
    ...
    public void check_test() throws Exception{
        ...
        emailCheckDTO.setEmail("sonny1234!");
        String requestBody = om.writeValueAsString(emailCheckDTO);
        User user = User.builder().id(1).roles("ROLE_USER").build();


        // stub
        Mockito.doNothing().when(userService).sameCheckEmail(any());
